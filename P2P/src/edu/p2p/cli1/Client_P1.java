...
public class Client_P1 {
	...
	private int ServerPort=21001;//1 send to 5
	private int ClientPort=21002;//2 send to 1
	static String RootFileLocation = "C:\\p2p";
	static String BaseFileLocation = RootFileLocation + "/ClientP1";
	...
	public static void main(String[] args) {
		Client_P1 Client = new Client_P1();
		...
		while(true)
		{
			//System.out.println("List for Client 1 to download:" + Client.chunkList);
			
			if(!Client.chunkList.isEmpty())
			{	
				// I change the function here to make it random
	...
	public void Connect_Client( int port) {
		try {
			...
			System.out.println("Client 1 Server socket created");
			while (true) {
				if (neighbour_c > 0) {
					neighbour_c--;
					SocketConnection = SocketReceive.accept();
					System.out.println("Client 1 connect with " + SocketConnection);
	...
	public void CreatConnection(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println("Client 1 connected to : " + "["+SocketClient.getPort()+"]");
	...
	public void ClientDisconnect() {
		try {
			inStream.close();
			SocketClient.close();
			System.out.println("Client 1 connection closed: "+"["+SocketClient.getPort()+"]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	//not CreateChunkToSend -> create receive chunk
...
class OwnerClientThread extends Thread {
	...
	public void run() {
		try {
			...
			/*Object array = (Object) arrayStream.readObject();//***************/
			...
			if(FileExist)
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives the request [" + ChunkTotalNumber + "] from [21005], accepted." );//*new
				send_chunk_object = build_chunk(CurrentChunk, ChunkTotalNumber);
			}
			else
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives the request [" + ChunkTotalNumber + "] from [21005], not accepted." );//*new
				send_chunk_object = build_chunk(null, -1);
			}
				sendObject(send_chunk_object);
				System.out.println("["+socket.getLocalPort()+"]"+ " Request [list to send] from [21005]");
			}		
			ServerDisconnect();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public synchronized Object build_chunk(File file, int chunkNum) throws IOException {
		...
		if(chunkNum>0)
		{
			...
			System.out.println("["+socket.getLocalPort()+"]"+" Sends chunk " + file.getName() + " to " + "[21005]" );//*new
	...
	public synchronized void ServerDisconnect() {
		try {
			outStream.close();
			socket.close();
			System.out.println("Client 1 - server socket closed : " + "[21005]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
