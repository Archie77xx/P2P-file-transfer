...
public class Client_P5 {
	...
	private int ServerPort=21005;
	private int ClientPort=21001;
	static String RootFileLocation = "C:\\p2p";
	static String BaseFileLocation = RootFileLocation + "/ClientP5";
	static String chunksLocation = BaseFileLocation + "/chunks";

	ObjectInputStream inStream;
	ObjectOutputStream outStream;
	/*ObjectOutputStream arrayStream;*/
	Set<Integer> chunkList;


	public static void main(String[] args) {
		Client_P5 Client = new Client_P5();
		...
		while(true)
		{
			//System.out.println("List for Client 5 to download:" + Client.chunkList);
			...
			if(!Client.chunkList.isEmpty())
			{
				...
				System.out.println(listregister);//*************cheeek
				/*Client.arrayStream.writeObject(listregister);*/
				/*Client.arrayStream.flush();*/
				for(int i=0;i<array.length;i++)
				{
	...
	public void Connect_Client( int port) {
		try {
			...
			System.out.println("Client 5 Server socket created");
			while (true) {
				if (neighbour_c > 0) {
					neighbour_c--;
					SocketConnection = SocketReceive.accept();
					System.out.println("Client 5 connect with " + SocketConnection);
	...
	public void CreatConnection(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println("Client 5 connected to : " + "["+SocketClient.getPort()+"]");
			inStream = new ObjectInputStream(SocketClient.getInputStream());
			outStream = new ObjectOutputStream(SocketClient.getOutputStream());
			/*arrayStream = new ObjectOutputStream(SocketClient.getOutputStream());*/
		}
	...
	public void ClientDisconnect() {
		try {
			inStream.close();
			SocketClient.close();
			System.out.println("Client 5 connection closed: "+"["+SocketClient.getPort()+"]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
class OwnerClientThread extends Thread {
	...
	public void run() {
		try {
			...
			/*arrayStream = new ObjectInputStream(socket.getInputStream());//**************/
			while(true)
			{
			int ChunkTotalNumber= (int)inStream.readObject();
			/*int array = (int)arrayStream.readObject();*/
			...
			for(int i=0;i< files.length;i++)
			{
				...
				/*System.out.println(array);//*************cheeek*/
			...
			if(FileExist)
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives request [" + ChunkTotalNumber + "] from [21004], accepted." );//*new
				send_chunk_object = build_chunk(CurrentChunk, ChunkTotalNumber);
			}
			else
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives request [" + ChunkTotalNumber + "] from [21004], not accepted." );//*new
				send_chunk_object = build_chunk(null, -1);
			}
				sendObject(send_chunk_object);
				System.out.println("["+socket.getLocalPort()+"]"+ " Requests [list to send] from [21004]");//
			}		
			ServerDisconnect();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public synchronized Object build_chunk(File file, int chunkNum) throws IOException {
		...
		if(chunkNum>0)
		{
			...
			System.out.println("["+socket.getLocalPort()+"]"+" Sends chunk " + file.getName() + " to " + "[21004]" );//
	...
	public synchronized void ServerDisconnect() {
		try {
			outStream.close();
			socket.close();
			System.out.println("Client 5 - server socket closed : " + "[21004]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
