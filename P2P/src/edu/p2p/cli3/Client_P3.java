...
public class Client_P3 {
	...
	private int ServerPort=21003;//1 send to 5
	private int ClientPort=21004;//2 send to 1
	static String RootFileLocation = "C:\\p2p";
	static String BaseFileLocation = RootFileLocation + "/ClientP3";
	...
	public static void main(String[] args) {
		Client_P3 Client = new Client_P3();
		...
		while(true)
		{
			//System.out.println("List for Client 3 to download:" + Client.chunkList);
	...
	public void Connect_Client( int port) {
		try {
			...
			System.out.println("Client 3 Server socket created");
			while (true) {
				if (neighbour_c > 0) {
					neighbour_c--;
					SocketConnection = SocketReceive.accept();
					System.out.println("Client 3 connect with " + SocketConnection);
	...
	public void CreatConnection(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println("Client 3 connected to : " + "["+SocketClient.getPort()+"]");
	...
	public void ClientDisconnect() {
		try {
			inStream.close();
			SocketClient.close();
			System.out.println("Client 3 connection closed: "+"["+SocketClient.getPort()+"]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
class OwnerClientThread extends Thread {
	...
	public void run() {
		try {
			...
			if(FileExist)
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives request [" + ChunkTotalNumber + "] from [21002], accepted." );//*new
				send_chunk_object = build_chunk(CurrentChunk, ChunkTotalNumber);
			}
			else
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives request [" + ChunkTotalNumber + "] from [21002], not accepted." );//*new
				send_chunk_object = build_chunk(null, -1);
			}
				sendObject(send_chunk_object);
				System.out.println("["+socket.getLocalPort()+"]"+ " Request [list to send] from [21002]");
			}		
			ServerDisconnect();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public synchronized Object build_chunk(File file, int chunkNum) throws IOException {
		...
		if(chunkNum>0)
		{
			...
			System.out.println("["+socket.getLocalPort()+"]"+" Sends chunk " + file.getName() + " to " + "[21002]" );
	...
	public synchronized void ServerDisconnect() {
		try {
			outStream.close();
			socket.close();
			System.out.println("Client 3 - server socket closed : " + "[21002]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
