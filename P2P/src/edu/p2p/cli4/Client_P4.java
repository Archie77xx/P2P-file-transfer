...
public class Client_P4 {
	...
	private int ServerPort=21004;
	private int ClientPort=21005;
	static String RootFileLocation = "C:\\p2p";
	static String BaseFileLocation = RootFileLocation + "/ClientP4";
	...
	public static void main(String[] args) {
		Client_P4 Client = new Client_P4();
		...
		while(true)
		{
			//System.out.println("List for Client 4 to download:" + Client.chunkList);
	...
	public void Connect_Client( int port) {
		try {
			...
			System.out.println("Client 4 Server socket created");
			while (true) {
				if (neighbour_c > 0) {
					neighbour_c--;
					SocketConnection = SocketReceive.accept();
					System.out.println("Client 4 connect with " + SocketConnection);
	...
	public void CreatConnection(int port) throws InterruptedException {
		...
		try {
			...
			System.out.println("Client 4 connected to : " + "["+SocketClient.getPort()+"]");
	...
	public void ClientDisconnect() {
		try {
			inStream.close();
			SocketClient.close();
			System.out.println("Client 4 connection closed: "+"["+SocketClient.getPort()+"]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
...
class OwnerClientThread extends Thread {
	...
	public void run() {
		try {
			...
			if(FileExist)
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives request [" + ChunkTotalNumber + "] from [21003], accepted." );//*new
				send_chunk_object = build_chunk(CurrentChunk, ChunkTotalNumber);
			}
			else
			{
				System.out.println("["+socket.getLocalPort()+"]"+" Receives request [" + ChunkTotalNumber + "] from [21003], not accepted." );//*new
				send_chunk_object = build_chunk(null, -1);
			}
				sendObject(send_chunk_object);
				System.out.println("["+socket.getLocalPort()+"]"+ " Request [list to send] from [21003]");
			}		
			ServerDisconnect();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public synchronized Object build_chunk(File file, int chunkNum) throws IOException {
		...
		if(chunkNum>0)
		{
			...
			System.out.println("["+socket.getLocalPort()+"]"+" Sends chunk " + file.getName() + " to " + "[21003]" );
	...
	public synchronized void ServerDisconnect() {
		try {
			outStream.close();
			socket.close();
			System.out.println("Client 4 - server socket closed : " + "[21003]");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
